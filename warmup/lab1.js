...
function generateRandomItemList(n) {
  let randomlist = [];
  let aplhabet = ["a", "b", "c", "d", "e", "f", "g"];

  for (let i = 0; i < n; i++) {
    randomizer = getRandomIntInclusive(1, 20);
    randomchar = getRandomIntInclusive(0, 6);
    randomlist.push({name: aplhabet[randomchar], cost: randomizer});
  }
  return randomlist;
}
...
const searchItems = (items, re) => {
  ...
  for (let i = 0; i < items.length; i++) {
    if((items[i].name).search(re) > -1) foundel.push(items[i]);
  }
  return foundel;
};
